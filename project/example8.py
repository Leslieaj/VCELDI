...
def parseEldi(Eldi):
	...
	observation = eldi[0]
	eldf = eldi[-1]
	eldfand = eldf.split('and')
	eldfchop = eldfand[0].split(';')
...
def e8chopcondition(spath, chopnum, ldis):
	...
	for chopcond in chopconditions:
		#print chopcond
		LDFsineqs = e8Ldfinequations(chopcond, spath.path, ldis)
...
def main():
	...
	for pps in allpaths:
		for sp in pps:
			...
			chopineqsstr = e8chopcondition(spath, chopnum, ldis)
